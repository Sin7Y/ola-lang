contract test {
	...
	fn uintMinD() public pure ->(bool) {
		...
		u256 uint256_min = type(u256).min;
		require(uint256_min == 0);

		return true;
	}

	fn uintMaxA() public pure -> (bool) {
	...
	fn uintMaxD() public pure ->(bool) {
		...
		u256 uint256_max = type(u256).max;
