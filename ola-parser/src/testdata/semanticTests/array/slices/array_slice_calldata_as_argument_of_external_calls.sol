contract C {
    fn f1(bytes calldata c1, uint256 s, uint256 e, bytes calldata c2) public returns (bool) {
        return keccak256(c1[s:e]) == keccak256(c2);
    }

    fn f2(bytes calldata c, uint256 s) public returns (uint256, bytes memory) {
        return abi.decode(c[s:], (uint256, bytes));
    }

    fn f3(bytes calldata c1, uint256 s, uint256 e, bytes calldata c2) public returns (bool) {
    ...
    fn f4(bytes calldata c1, uint256 s, uint256 e, bytes calldata c2) public returns (bool) {
