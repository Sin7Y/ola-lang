contract C {
    ...
    fn f(bytes memory a) public returns (bytes memory) {
        return bytes.concat(a, "bcdef");
    }
    fn g(bytes calldata a) public returns (bytes memory) {
        return bytes.concat(a, "abcdefghabcdefghabcdefghabcdefghab");
    }
    fn h(bytes calldata a) public returns (bytes memory) {
        return bytes.concat(a, s);
    }
    fn j(bytes calldata a) public returns (bytes memory) {
        bytes storage ref = s;
        return bytes.concat(a, ref, s);
    }
    fn k(bytes calldata a, string memory b) public returns (bytes memory) {
        return bytes.concat(a, bytes(b));
    }
    fn slice(bytes calldata a) public returns (bytes memory) {
        require(a.length > 2, "");
        return bytes.concat(a[:2], a[2:]);
    }
    fn strParam(string calldata a) public returns (bytes memory) {
        return bytes.concat(bytes(a), "bcdef");
    }
    fn fixedBytesParam(bytes16 b1, bytes15 b2, bytes31 b3) public returns (
        bytes memory,
        bytes memory,
        bytes memory,
        bytes memory
    ...
    fn fixedBytesParam2(bytes calldata c, bytes6 b1, bytes6 b2) public returns (bytes memory, bytes memory) {
        return (
            bytes.concat(s, b1, c),
