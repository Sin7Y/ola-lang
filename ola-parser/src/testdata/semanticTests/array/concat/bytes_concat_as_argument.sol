contract C {
    fn f(bytes memory a, bytes memory b) public returns (bytes32) {
        return keccak256(bytes.concat(a, b));
    }

    fn h(bytes memory a) internal returns (uint256) {
        return a.length;
    }

    fn g(bytes memory a, bytes memory b) public returns (uint256) {
