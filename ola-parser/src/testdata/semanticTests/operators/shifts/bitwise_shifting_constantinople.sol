contract C {
    fn shl(uint256 a, uint256 b) public returns (uint256 c) {
        assembly {
            c := shl(b, a)
        }
    }

    fn shr(uint256 a, uint256 b) public returns (uint256 c) {
        assembly {
            c := shr(b, a)
        }
    }

    fn sar(uint256 a, uint256 b) public returns (uint256 c) {
        assembly {
            c := sar(b, a)
        }
    }
