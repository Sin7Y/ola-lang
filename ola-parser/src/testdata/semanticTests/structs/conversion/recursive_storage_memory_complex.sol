contract CopyTest {
    ...
    fn countData(Tree memory tree) internal -> (u256 c) {
        c = 1;
    ...
    fn copyFromTree(Tree memory tree, u256[] memory data, u256 offset) internal -> (u256) {
    ...
    fn run() public -> (u256[] memory) {
        Tree memory memoryTree;
        memoryTree = storageTree;
        u256 length = countData(memoryTree);
        u256[] memory result = new u256[](length);
