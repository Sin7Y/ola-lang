...
abstract contract Registrar {
	...
	fn owner(string memory _name) public virtual view returns (address o_owner);
	fn addr(string memory _name) public virtual view returns (address o_address);
	fn subRegistrar(string memory _name) virtual public view returns (address o_subRegistrar);
	fn content(string memory _name) public virtual view returns (bytes32 o_content);
}
...
contract FixedFeeRegistrar is Registrar {
	...
	fn reserve(string memory _name) public payable {
	...
	fn disown(string memory _name, address payable _refund) onlyrecordowner(_name) public {
		delete m_recordData[uint(keccak256(bytes(_name))) / 8];
		if (!_refund.send(c_fee))
			revert();
		emit Changed(_name);
	}
	fn transfer(string memory _name, address _newOwner) onlyrecordowner(_name) public {
		m_record(_name).owner = _newOwner;
		emit Changed(_name);
	}
	fn setAddr(string memory _name, address _a) onlyrecordowner(_name) public {
		m_record(_name).addr = _a;
		emit Changed(_name);
	}
	fn setSubRegistrar(string memory _name, address _registrar) onlyrecordowner(_name) public {
		m_record(_name).subRegistrar = _registrar;
		emit Changed(_name);
	}
	fn setContent(string memory _name, bytes32 _content) onlyrecordowner(_name) public {
		m_record(_name).content = _content;
		emit Changed(_name);
	}

	fn record(string memory _name) public view returns (address o_addr, address o_subRegistrar, bytes32 o_content, address o_owner) {
		Record storage rec = m_record(_name);
		o_addr = rec.addr;
	...
	fn addr(string memory _name) public override view returns (address) { return m_record(_name).addr; }
	fn subRegistrar(string memory _name) public override view returns (address) { return m_record(_name).subRegistrar; }
	fn content(string memory _name) public override view returns (bytes32) { return m_record(_name).content; }
	fn owner(string memory _name) public override view returns (address) { return m_record(_name).owner; }

	Record[2**253] m_recordData;
	fn m_record(string memory _name) view internal returns (Record storage o_record) {
