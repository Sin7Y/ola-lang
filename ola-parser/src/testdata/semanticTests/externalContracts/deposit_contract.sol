...
interface IDepositContract {
    ...
    fn get_deposit_root() external view returns (bytes32);

    /// @notice Query the current deposit count.
    /// @return The deposit count encoded as a little endian 64-bit number.
    fn get_deposit_count() external view returns (bytes memory);
}
...
interface ERC165 {
    ...
    fn supportsInterface(bytes4 interfaceId) external pure returns (bool);
}
...
contract DepositContract is IDepositContract, ERC165 {
    ...
    fn get_deposit_root() override external view returns (bytes32) {
        bytes32 node;
    ...
    fn get_deposit_count() override external view returns (bytes memory) {
    ...
    fn supportsInterface(bytes4 interfaceId) override external pure returns (bool) {
        return interfaceId == type(ERC165).interfaceId || interfaceId == type(IDepositContract).interfaceId;
    }

    fn to_little_endian_64(uint64 value) internal pure returns (bytes memory ret) {
        ret = new bytes(8);
