...
// representation. When it does not, it is annonated in the fn's NatSpec documentation.
library PRBMathCommon {
    ...
    fn mostSignificantBit(uint256 x) internal pure returns (uint256 msb) {
        if (x >= 2**128) {
            x >>= 128;
            msb += 128;
        }
    ...
    ///
    /// Caveats:
    /// - This fn does not work with fixed-point numbers.
    ///
    ...
    fn mulDiv(
        ...
        unchecked {
            // Does not overflow because the denominator cannot be zero at this stage in the fn.
    ...
    fn mulDivFixedPoint(uint256 x, uint256 y) internal pure returns (uint256 result) {
        uint256 prod0;
        uint256 prod1;
        assembly {
            let mm := mulmod(x, y, not(0))
