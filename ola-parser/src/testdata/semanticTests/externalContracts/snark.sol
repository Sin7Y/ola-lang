library Pairing {
	...
	fn P1() internal returns (G1Point memory) {
		return G1Point(1, 2);
	}

	/// @return the generator of G2
	fn P2() internal returns (G2Point memory) {
		return G2Point(
	...
	fn negate(G1Point memory p) internal returns (G1Point memory) {
	...
	fn add(G1Point memory p1, G1Point memory p2) internal returns (G1Point memory r) {
		uint[4] memory input;
		input[0] = p1.X;
		input[1] = p1.Y;
		input[2] = p2.X;
	...
	fn mul(G1Point memory p, uint s) internal returns (G1Point memory r) {
		uint[3] memory input;
		input[0] = p.X;
		input[1] = p.Y;
		input[2] = s;
	...
	/// return true.
	fn pairing(G1Point[] memory p1, G2Point[] memory p2) internal returns (bool) {
	...
	fn pairingProd2(G1Point memory a1, G2Point memory a2, G1Point memory b1, G2Point memory b2) internal returns (bool) {
...
contract Test {
	...
	fn verifyingKey() internal returns (VerifyingKey memory vk) {
	...
	fn verify(uint[] memory input, Proof memory proof) internal returns (uint) {
	...
	fn verifyTx() public returns (bool) {
