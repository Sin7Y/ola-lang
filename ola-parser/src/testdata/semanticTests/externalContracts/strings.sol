...
contract test {
    ...
    fn toSlice(string memory a) external pure returns (strings.slice memory) {
        return a.toSlice();
    }

    fn roundtrip(string memory a) external pure returns (string memory) {
        return a.toSlice().toString();
    }

    fn utf8len(string memory a) external pure returns (uint) {
        return a.toSlice().len();
    }

    fn multiconcat(string memory a, uint count) public pure returns (string memory) {
    ...
    fn benchmark(string memory text, bytes32 seed) external pure returns (uint) {
        // Grow text.
