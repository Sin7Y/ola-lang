contract C {
	fn f(int a) public {}
	fn f3(int a, int b) public {}

	fn failFunctionArgsWrongType() public returns(bytes memory) {
		return abi.encodeCall(this.f, ("test"));
	}
	fn failFunctionArgsTooMany() public returns(bytes memory) {
		return abi.encodeCall(this.f, (1, 2));
	}
	fn failFunctionArgsTooFew0() public returns(bytes memory) {
		return abi.encodeCall(this.f, ());
	}
	fn failFunctionArgsTooFew1() public returns(bytes memory) {
		return abi.encodeCall(this.f);
	}
	fn failFunctionArgsArrayLiteral() public returns(bytes memory) {
...
// TypeError 6219: (489-511): Expected two arguments: a fn pointer followed by a tuple.
