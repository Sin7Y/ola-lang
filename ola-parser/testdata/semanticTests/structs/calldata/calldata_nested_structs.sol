...
contract C {
    ...
    fn f1(S1  c) internal ->(S1 ) {
        return c;
    }

    fn f(S1  c, uint32 p) external ->(uint32, uint128, u256, u256, uint32) {
        S1 memory m = f1(c);
    ...
    fn g(S2  c) external ->(uint128, u256, u256, uint32) {
        S2 memory m = c;
    ...
    fn h(S1  c, uint32 p) external ->(uint32, uint128, u256, u256, uint32) {
        S memory m = c.s;
