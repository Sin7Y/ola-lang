...
Ola 为静态类型语言，变量类型须在在编译期已知，可以避免大部分的运行时异常。
...
Ola 提供基于field实现的上述的各种整数类型基础libs，便于开发者编写复杂逻辑。
...
`field`是 Ola 的最基本类型，`field`变量的数据范围为`[0, p-1]`。

olaVM使用plonky2作为其后端证明系统，故`field`的素数域选择为`$p=2^{64}-2^{32}+1$`。

对于Ola开发者，`field`类型操作与整数类型一样，在overflow时需要特别注意。
...
Ola支持多种复杂类型，如`Arrays`、`Slice`、`Tuples`、`Structs`、`Enumerations`、`Map`。

##### Arrays

Ola支持静态类型数组，数组元素的数据类型必须保持一致，且数组大小须在编译期确定。
