...
impl Namespace {
        ...
        for base in self.contracts[contract_no].bases.iter() {
                    ...
                    .get(&(file_no, Some(base.contract_no), id.name.to_owned()))
            {
                if let Symbol::Variable(_, var_contract_no, var_no) = sym {
                    ...
                    if let program::Visibility::Private(_) = var.visibility {
                        return None;
                    }
                }
                ...
                program::Type::Function {
                    ...
                    for a in attributes {
                        match a {
                            ...
                            program::FunctionAttribute::Visibility(v @ program::Visibility::Internal(_))
                            | program::FunctionAttribute::Visibility(v @ program::Visibility::External(_))
                    ...
                    let is_external = match visibility {
                        None | Some(program::Visibility::Internal(_)) => false,
                        Some(program::Visibility::External(_)) => true,
                        Some(v) => {
                            diagnostics.push(Diagnostic::error(
                                v.loc().unwrap(),
                    ...
                    let mutability = match mutability {
                        ...
                        Some(program::Mutability::Payable(loc)) => Mutability::Payable(loc),
                        Some(program::Mutability::Pure(loc)) => Mutability::Pure(loc),
                        Some(program::Mutability::View(loc)) => Mutability::View(loc),
                        Some(program::Mutability::Constant(loc)) => Mutability::View(loc),
                    };
    ...
    pub fn expr_to_identifier_path(&self, mut expr: &program::Expression) -> Option<program::IdentifierPath> {
        let loc = expr.loc();
