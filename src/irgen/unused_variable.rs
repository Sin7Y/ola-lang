...
/// Check if we should remove an assignment. The expression in the argument is
...
pub fn should_remove_assignment(ns: &Namespace, exp: &Expression, func: &Function) -> bool {
    match &exp {
        ...
        Expression::Variable(_, _, var_no) => should_remove_variable(*var_no, func),

        Expression::StructMember(_, _, expr, _) => should_remove_assignment(ns, expr, func),

        Expression::Subscript(_, _, _, array, _) => should_remove_assignment(ns, array, func),
...
pub fn should_remove_variable(pos: usize, func: &Function) -> bool {
    ...
    //If the variable has never been read nor assigned, we can remove it right
    // away.
