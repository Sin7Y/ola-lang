...
use crate::sema::ast::{ArrayLength, DestructureField, Expression, Function, Namespace, RetrieveType, Statement, Type};
...
pub(crate) fn statement<'a>(
    ...
    match stmt {
        ...
        Statement::VariableDecl(_, pos, param, Some(init)) => {
            ...
            func_context.var_table.insert(*pos, alloc.as_basic_value_enum());
            bin.builder
        ...
        Statement::VariableDecl(loc, pos, param, None) => {
            ...
            if matches!(param.ty, Type::Array(..)) {
                ...
                let alloc =
                    bin.build_alloca(func_context.func_val, bin.llvm_var_ty(&Uint(32), ns), "array_length");
        ...
        Statement::If(_, _, cond, then_stmt, else_stmt) => if_then_else(
            cond, then_stmt, else_stmt, bin, func_context, ns,
        ),
        ...
        Statement::For {
            ...
            let cond_block = bin.context.append_basic_block(func_context.func_val, "cond");
            let body_block = bin.context.append_basic_block(func_context.func_val, "body");
            let next_block = bin.context.append_basic_block(func_context.func_val, "next");
            let end_block = bin.context.append_basic_block(func_context.func_val, "endfor");
...
fn if_then<'a>(
    ...
    let then = bin.context.append_basic_block(func_context.func_val, "then");
    let endif = bin.context.append_basic_block(func_context.func_val, "enif");
...
fn if_then_else<'a>(
    ...
    let then = bin.context.append_basic_block(func_context.func_val, "then");
    let else_ = bin.context.append_basic_block(func_context.func_val, "else");
    let endif = bin.context.append_basic_block(func_context.func_val, "enif");
    bin.builder.position_at_end(pos);
    ...
    let value = match expr {
        ...
        Expression::List(_, list) => {
            ...
            let struct_type = bin.context.struct_type(
                &res.iter().map(|value| value.get_type()).collect::<Vec<BasicTypeEnum>>(),
                false,
            );
            // Allocate the struct
            ...
            for (index, value) in res.into_iter().enumerate() {
                let field_ptr = bin.builder.build_struct_gep(struct_ptr, index as u32, &format!("field_{}", index));
...
fn destructure<'a>(
    ...
    if let Expression::ConditionalOperator(_, _, cond, left, right) = expr {
        ...
        bin.builder.build_phi(bin.llvm_type(&expr.ty(), ns), "phi");
    ...
    let mut value = match expr {
        ...
        // We need to return a struct, which corresponds to handling multiple return values in functions.
        Expression::List (_, list) => {
            ...
            let struct_member_types: Vec<BasicTypeEnum> = values
                .iter()
            ...
            for (i, value) in values.iter().enumerate() {
                let field_ptr = bin
                    .builder
                    .build_struct_gep(struct_alloca, i as u32, &format!("field_ptr{}", i));
                bin.builder.build_store(field_ptr.unwrap(), *value);
            }
            struct_alloca.into()
    ...
    for (i, field) in fields.iter().enumerate() {
        ...
        match field {
            ...
            DestructureField::VariableDecl(res, param) => {
                ...
                func_context.var_table.insert(*res, alloc.as_basic_value_enum());
                if is_single_value {
                    bin.builder
                    ...
                    let field = bin.builder.build_extract_value(
