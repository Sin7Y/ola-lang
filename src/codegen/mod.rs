...
use ola_parser::program::{CodeLocation, FunctionTy};
...
pub enum Expression {
    ...
    AllocDynamicBytes(program::Loc, Type, Box<Expression>, Option<Vec<u8>>),
    ...
    BytesCast(program::Loc, Type, Type, Box<Expression>),
    BytesLiteral(program::Loc, Type, Vec<u8>),
    Cast(program::Loc, Type, Box<Expression>),
    CodeLiteral(program::Loc, usize, bool),
    Complement(program::Loc, Type, Box<Expression>),
    ConstArrayLiteral(program::Loc, Type, Vec<u32>, Vec<Expression>),
    UnsignedDivide(program::Loc, Type, Box<Expression>, Box<Expression>),
    SignedDivide(program::Loc, Type, Box<Expression>, Box<Expression>),
    Equal(program::Loc, Box<Expression>, Box<Expression>),
    FormatString(program::Loc, Vec<(FormatArg, Expression)>),
    FunctionArg(program::Loc, Type, usize),
    GetRef(program::Loc, Type, Box<Expression>),
    InternalFunctionCfg(usize),
    Keccak256(program::Loc, Type, Vec<Expression>),
    List(program::Loc, Vec<Expression>),
    SignedLess(program::Loc, Box<Expression>, Box<Expression>),
    UnsignedLess(program::Loc, Box<Expression>, Box<Expression>),
    LessEqual(program::Loc, Box<Expression>, Box<Expression>),
    Load(program::Loc, Type, Box<Expression>),
    UnsignedModulo(program::Loc, Type, Box<Expression>, Box<Expression>),
    SignedModulo(program::Loc, Type, Box<Expression>, Box<Expression>),
    SignedMore(program::Loc, Box<Expression>, Box<Expression>),
    UnsignedMore(program::Loc, Box<Expression>, Box<Expression>),
    ...
    RationalNumberLiteral(program::Loc, Type, BigRational),
    ReturnData(program::Loc),
    SignExt(program::Loc, Type, Box<Expression>),
    ...
    Trunc(program::Loc, Type, Box<Expression>),
    UnaryMinus(program::Loc, Type, Box<Expression>),
    Undefined(Type),
    Variable(program::Loc, Type, usize),
    ZeroExt(program::Loc, Type, Box<Expression>),
    AdvancePointer {
...
impl CodeLocation for Expression {
    fn loc(&self) -> program::Loc {
        match self {
            ...
            | Expression::AdvancePointer { .. } => program::Loc::Codegen,
        }
    }
}
