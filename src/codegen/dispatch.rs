...
use crate::{
    sema::ast::{ArrayLength, Namespace, Parameter, Type},
};
use num_bigint::{BigInt, Sign};
use num_traits::{ToPrimitive, Zero};
use ola_parser::program;
use ola_parser::program::{FunctionTy, Loc};
use std::sync::Arc;
...
pub(super) fn function_dispatch(
    ...
    for (cfg_no, func_cfg) in all_cfg.iter().enumerate() {
        if func_cfg.ty != program::FunctionTy::Function || !func_cfg.public {
            continue;
        }
    ...
    let fallback = all_cfg
        .iter()
        .enumerate()
        .find(|(_, cfg)| cfg.public && cfg.ty == program::FunctionTy::Fallback);

    let receive = all_cfg
        .iter()
        .enumerate()
        .find(|(_, cfg)| cfg.public && cfg.ty == program::FunctionTy::Receive);
...
fn solana_deploy(
    ...
    cfg.add(
        ...
        Instr::SetStorage {
            ...
            storage: Expression::NumberLiteral(program::Loc::Codegen, Type::Uint(64), BigInt::zero()),
        },
    );
    ...
    cfg.add(
        ...
        Instr::SetStorage {
            ...
            storage: Expression::NumberLiteral(program::Loc::Codegen, Type::Uint(64), BigInt::from(12)),
        },
    );
}
