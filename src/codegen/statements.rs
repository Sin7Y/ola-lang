    ...
    match stmt {
        ...
        Statement::VariableDecl(loc, pos, param, None) => {
            ...
            if matches!(param.ty, Type::Array(..)) {
                let num =
                    Expression::NumberLiteral(program::Loc::Codegen, Type::Uint(32), BigInt::zero());
                let temp_res = vartab.temp_name("array_length", &Type::Uint(32));
                cfg.add(
                    vartab,
                    Instr::Set {
        ...
        ast::Expression::ExternalFunctionCall {
            ...
            } = function.ty()
            {
                let value = if let Some(value) = &call_args.value {
                    ...
                    Expression::NumberLiteral(program::Loc::Codegen, Type::Value, BigInt::zero())
                };
                ...
                if func_returns != vec![Type::Void] {
                    ...
                    let tys = func_returns
                        ...
                        .map(|ty| Parameter {
                            ty: ty.clone(),
                            id: None,
                            ty_loc: Some(program::Loc::Codegen),
                    ...
                    cfg.add(
                        ...
                        Instr::AbiDecode {
                            ...
                            tys,
                            data: Expression::ReturnData(program::Loc::Codegen),
                            data_len: None,
                        },
                    );
                }
    ...
    if let Some(res) = try_stmt.catch_param_pos {
        cfg.add(
            ...
            Instr::Set {
                loc: program::Loc::Codegen,
                res,
                expr: Expression::ReturnData(program::Loc::Codegen),
            },
        );
    }
    ...
    pub fn default(&self, ns: &Namespace) -> Option<Expression> {
        match self {
            ...
            Type::Bytes(n) => {
                ...
                Some(Expression::BytesLiteral(program::Loc::Codegen, self.clone(), l))
            }
...
impl Namespace {
    ...
    pub fn default_constructor(&self, contract_no: usize) -> Function {
        ...
        func.body = vec![Statement::Return(program::Loc::Codegen, None)];
        func.has_body = true;
