...
fn interfaceid(ns: &Namespace, contract_no: &usize, loc: &program::Loc) -> Expression {
        ...
        _ => {
            ...
            let cfg_right =
                if !left_ty.is_contract_storage() && cfg_right.ty().is_fixed_reference_type() {
                    Expression::Load(program::Loc::Codegen, cfg_right.ty(), Box::new(cfg_right))
                } else {
                    cfg_right
                };
    ...
    match expr {
        ast::Expression::InternalFunctionCall { function, args, .. } => {
            ...
            } = function.as_ref()
            {
                ...
                let call = if ns.functions[function_no].loc == program::Loc::Builtin {
