...
use crate::ir::program::{BasicBlock, BasicBlockData, Function, Program, Value, ValueData};
...
pub trait ValueBuilder: Sized + EntityInfoQuerier + ValueInserter {
    ...
    fn integer(mut self, value: u32) -> Value {
        self.insert_value(Uint32::new_data(value))
    }
...
pub trait LocalInstBuilder: ValueBuilder {
    ...
    fn branch(mut self, cond: Value, true_bb: BasicBlock, false_bb: BasicBlock) -> Value {
        assert!(self.value_type(cond).is_U32(), "`cond` must be integer");
        assert!(
